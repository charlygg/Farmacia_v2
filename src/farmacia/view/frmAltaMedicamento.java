...
public class frmAltaMedicamento extends JFrame implements ActionListener {

	private JTextField txt_nombre, txt_codigo, txt_contenido, txt_farmaco,
			txt_pres, txt_precio, txt_existencia;
	@SuppressWarnings("rawtypes")
	private JComboBox cb_tipo, cb_pasillo, cb_estante, cb_repisa, cb_marca;
	...
	public frmAltaMedicamento() {
		...
		txt_pres = new JTextField();
		txt_pres.setBounds(139, 160, 86, 20);
		pnlInfo.add(txt_pres);
		txt_pres.setFont(new Font("Albertus MT", Font.PLAIN, 12));
		...
		btnValidar.setIcon(new ImageIcon("icons/Select.png"));
		btnValidar.setFont(new Font("Calisto MT", Font.PLAIN, 14));
		btnValidar.setBounds(258, 17, 130, 32);
		btnValidar.addActionListener(this);
		pnlInfo.add(btnValidar);
		...
		setVisible(true);
		setFondo("Fondo de medicina.jpg");
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btnValidar) {
			int res;
			try {
				res = MedicamentoImp.isClaveExists(txt_codigo.getText());
				if (res == 1) {
					JOptionPane.showOptionDialog(rootPane,
							"Esta clave ya está en uso, elejir otra", "Error",
							JOptionPane.DEFAULT_OPTION,
							JOptionPane.WARNING_MESSAGE, null, null, null);
				} else {
					JOptionPane.showMessageDialog(rootPane,
							"Esa clave esta disponible");
				}
		...
		if (txt_nombre.getText().equals("") || txt_codigo.getText().equals("")
			...
			if (respuesta) {
				int idUbicacion = ubicacion.getIdUbicacion(pasillo, estante,
						repisa);
				// validarcod();
				if (MedicamentoImp.isClaveExists(txt_codigo.getText()) != 1) {
					/* Obtencion de datos */
					...
					String presentacionw = txt_pres.getText();
					String  existenciaw = txt_existencia.getText();

					try {
						/* Asingacion de datos y subida*/
						...
						if(med.insertMedicamento(medicamento) == 1){
							JOptionPane.showMessageDialog(rootPane,
									"Guardado con éxito");
							this.dispose();
						}
					...
					JOptionPane.showOptionDialog(rootPane,
							"Clave no válida, está siendo usada", "Error",
							JOptionPane.DEFAULT_OPTION,
