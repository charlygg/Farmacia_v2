...
public class frmConsultaMedicamento extends JFrame implements ActionListener {
	...
	public frmConsultaMedicamento() {
		...
		btnInfo.setBounds(202, 557, 125, 36);
		...
		cbBusqueda = new JComboBox<Object>(new String[] {"Nombre","Clave","Precio","Marca","Farmaco",
				"Pasillo","Estante","Repisa"});
		cbBusqueda.setFont(new Font("Calisto MT", Font.PLAIN, 16));
		cbBusqueda.setBounds(10, 18, 121, 28);
		panelBusqueda.add(cbBusqueda);

		btnBaja = new JButton("Baja");
		btnBaja.setIcon(new ImageIcon("icons/build.png"));
		btnBaja.setFont(new Font("Calisto MT", Font.PLAIN, 18));
		btnBaja.setBounds(44, 557, 125, 36);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btnBaja){
	        int ax = JOptionPane.showConfirmDialog(null, "¿Estas seguro de eliminar estos registros?");
	        if(ax == JOptionPane.YES_OPTION){
	            ...
	            for (int i = 0; i < table.getRowCount(); i++) {
	                ...
	                if (isChecked) {
						...
						try {
							countDeleted = countDeleted + medicamentoImp.deleteMedicamento(clave);
	            ...
	            if(countDeleted > 0){
	            	JOptionPane.showMessageDialog(null, "Medicamentos eliminados: " + countDeleted);
	            	crearTabla((String) comboBox.getSelectedItem());
	            }
	        }
	...
	private void setFiltro(String filtro){
		ArrayList<Medicamento> meds = new ArrayList<Medicamento>();
		try {
			meds = medicamentoImp.filtrarTabla(filtro.toLowerCase(),txtBusqueda.getText());
			model = new DefaultTableModel(null, new String[] { "Clave",
					"Nombre", "Precio", "Marca", "Pasillo", "Estante",
					"Repisa", "Seleccion" });
			for (Medicamento m : meds) {
	...
	private void getInfo(){
        ...
        if(listadosMedicamentos != null) new frmConsultaInfo(listadosMedicamentos);
	}
}